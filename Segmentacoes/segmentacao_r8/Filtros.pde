...
class Filtros {
    ...
    for (int y=0; y<img.height; y++) {
      for (int x=0; x<img.width; x++) {
        ...
        if (red > blue && red > green) {
          imgFiltrada.pixels[pos] = color(red, 0, 0);
        } else if (blue > red && blue > green) {
          imgFiltrada.pixels[pos] = color(0, 0, blue);
        } else if (green > red && green > blue) {
          imgFiltrada.pixels[pos] = color(0, green, 0);
        } else {
  ...
  // Método para realizar o filtro de Mediana
